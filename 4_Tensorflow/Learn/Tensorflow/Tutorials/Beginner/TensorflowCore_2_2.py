...
########################--Load the Data -- Make a split
#Sentiment analysis--classify movie reviews as positive or negative, based on the text of the review. 
#Movie Review Dataset contains 50,000 movie reviews
#Split into 25,000 (train) vs 25,000 (test)
#The training/testing sets are balanced

#url = "https://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz"
#dataset = tf.keras.utils.get_file("aclImdb_v1", url, untar=True, cache_dir='.',cache_subdir='')
#dataset_dir = os.path.join(os.path.dirname(dataset), 'aclImdb')

dataset_dir="./datasets/aclImdb/"  #in subsequent calls
...
#Dataset divided into train and test, but it lacks a validation set. 
...
#use the remaining 5,000 reviews from the training set for validation.
#When using the validation_split and subset arguments, either specify a random seed, or pass shuffle=False, so that the validation and training splits have no overlap.
...
raw_test_ds = tf.keras.utils.text_dataset_from_directory('./datasets/aclImdb/train', batch_size=batch_size)


########################--Prepare the dataset for training

#standardize, tokenize, and vectorize with tf.keras.layers.TextVectorization layer.
...
'''
#Plotting -- Create a plot of accuracy and loss over time
...
print("Accuracy after Exporting on Raw data -- ", accuracy)

#Inference on new data
examples = ["The movie was great!", "The movie was okay.", "The movie was terrible..."]
print("Accuracy after Exporting on New data -- ", export_model.predict(examples))
