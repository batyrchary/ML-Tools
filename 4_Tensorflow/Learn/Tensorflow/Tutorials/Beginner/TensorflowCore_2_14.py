...
#pip install "tensorflow-text==2.13.*"  #not running
...
binary_model = tf.keras.Sequential([binary_vectorize_layer, layers.Dense(4)])
...
#You will map tokens to integers with 0 reserved for padding, and n+1 reserved to denote an out-of-vocabulary (OOV) token.
...
preprocess_text = tf.keras.Sequential([tokenizer, vocab_table])
...
#Export the model
export_model = tf.keras.Sequential([preprocess_text, model])
